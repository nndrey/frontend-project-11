...
export default (watcheState, i18n, path, val) => {
  ...
  if (path === 'feeds') {
    if (feedBox.children.length === 0) feedBox.append(renderFeeds(watcheState.feeds));
    const ul = document.querySelector('.feeds .list-group');
    ul.textContent = '';
    watcheState.feeds.forEach((feed) => {
      const li = document.createElement('li');
      li.classList.add('list-group-item', 'border-0', 'border-end-0');
      const h3 = document.createElement('h3');
      h3.classList.add('h6', 'm-0');
  ...
  if (path === 'posts' && watcheState.formData.validation) {
    if (postBox.children.length === 0) postBox.append(renderPosts(watcheState.posts));
    const ul = document.querySelector('.posts .list-group');
    ul.textContent = '';
    watcheState.posts.forEach((post) => {
      const li = document.createElement('li');
      li.classList.add('list-group-item', 'd-flex', 'justify-content-between', 'align-items-start', 'border-0', 'border-end-0');
      const aElem = document.createElement('a');
      aElem.classList.add('fw-bold');
